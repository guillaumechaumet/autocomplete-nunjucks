...
describe "nunjucks autocompletions", ->
  [editor, provider] = []
  ...
  beforeEach ->
    waitsForPromise -> atom.packages.activatePackage('autocomplete-nunjucks')

    runs ->
      provider = atom.packages.getActivePackage('autocomplete-nunjucks').mainModule.getProvider()
  ...
  Object.keys(packagesToTest).forEach (packageLabel) ->
    if packagesToTest[packageLabel].name in ['language-sass', 'language-less']
      describe "#{packageLabel} files", ->
        ...
        # pseudo-selectors are meta.property-value.snunjucks/less
        xit "autocompletes pseudo selectors when nested in LESS and Snunjucks files", ->
          editor.setText """
            .some-class {
              .a:f
            }
